...
**Versão:** 1.7  
**Vigente desde:** 18 de maio de 2026  
...
A **SIAS - Soluções em Inteligência Artificial e Sustentabilidade LTDA** ("**Vox Civis**", "**nós**", "**a Empresa**") opera plataforma SaaS de **inteligência territorial com inteligência artificial multi-agente**, que integra dados oficiais públicos, percepção social e indicadores territoriais, com cobertura dos **5.571 municípios brasileiros**.

Nossa missão é **transformar dados públicos fragmentados em inteligência territorial acessível**, para que decisões que impactam milhões de pessoas sejam fundamentadas em evidência verificável.

A plataforma opera por meio de **5 agentes de IA especializados**, cada um com escopo funcional próprio:

- **VOX RADAR** — *Radiografia territorial com dados oficiais.* Cruza 21 fontes oficiais, mapeia demandas por frequência e intensidade, segmenta por município, região e estado, e gera recomendações fundamentadas em evidências. Trabalha exclusivamente com dados públicos, sem informação pessoalmente identificável (PII).

- **VOX PULSO** — *Percepção social em tempo real.* Análise de sentimento e tendências em tempo real, com robustez amostral. Opera sempre em base **agregada**, sem identificação individual.

- **VOX CONEXÃO** — *Correlações invisíveis.* Cruza dados oficiais × percepção social para revelar alinhamentos e desalinhamentos invisíveis. Adota princípio de **anti-causalidade**: apresenta correlações, não relações causais sem evidência.

- **VOX MENSAGEM** — *Narrativas baseadas em evidências.* Transforma análises em **narrativas institucionais** prontas, com fonte rastreável para cada afirmação. Não personaliza conteúdo por perfil individual (proteção contra microtargeting · alinhado à Lei 9.504/97).

- **VOX GUARDIÃO** — *LGPD, fontes e riscos.* Camada de governança que classifica riscos em 4 níveis com base legal e mantém trilha de auditoria completa por 5 anos. Verifica 100% das saídas dos demais agentes.

Cada agente processa dados com **isolamento funcional**, sem acesso transversal automático. Dados territoriais são de fontes públicas (IBGE, DataSUS, INEP, TSE, FINBRA, INPE, entre outros). **Metadados de uso** (quem consultou o quê, quando) são tratados como confidenciais e nunca compartilhados sem consentimento.
...
A segurança e a proteção dos dados são pilares fundamentais da plataforma VoxCivis, integrados desde a concepção (*Security by Design*). A SIAS adota controles alinhados às melhores práticas de mercado e evolui continuamente sua postura de segurança conforme o amadurecimento da operação.

### 9.1 Princípios e controles em produção

A arquitetura VoxCivis incorpora os seguintes pilares de segurança:

- **Identidade gerenciada** em servidor de autenticação dedicado, no padrão **OAuth2 / OpenID Connect** — referência consolidada em instituições financeiras e plataformas governamentais
- **Criptografia em trânsito** (TLS) com renovação automática de certificados em todos os endpoints públicos
- **Auditoria completa** de operações sensíveis em log com retenção de **5 anos**, conforme exigência LGPD
- **Controle de tráfego** com rate-limiting em múltiplas camadas, protegendo contra abuso e exfiltração massiva
- **Gate de compliance automatizado (VOX GUARDIÃO)** — toda saída de IA passa por verificação automática de conformidade (LGPD e Lei 9.504/97), com watermark de autoria
- **Princípio do menor privilégio** aplicado a todos os acessos administrativos
- **Soberania de dados:** infraestrutura hospedada em território nacional brasileiro

### 9.2 Frameworks de referência adotados

A arquitetura VoxCivis tem como referência os principais frameworks consolidados de mercado:

- **LGPD** (Lei 13.709/2018) — base legal da proteção de dados pessoais
- **Marco Civil da Internet** (Lei 12.965/2014)
- **OWASP Top 10** — referência para mitigação de vulnerabilidades em aplicação web
...
A SIAS mantém procedimento formal de resposta a incidentes de segurança alinhado ao art. 48 da LGPD:
...
A SIAS valoriza pesquisadores de segurança e a comunidade técnica. Vulnerabilidades identificadas devem ser reportadas a <dpo@voxcivis.ai> sob disclosure responsável: não exploramos, não retaliamos, e reconhecemos colaborações relevantes.
...
Quando aplicável, a VoxCivis utiliza técnicas reconhecidas de anonimização e mascaramento de dados pessoais, conforme padrões da indústria, garantindo a irreversibilidade exigida pelo art. 12 da LGPD.
...
**Componentes auto-hospedados (não são operadores externos).** Demais componentes da plataforma (autenticação, banco de dados, cache, orquestração interna) são software de código aberto executados em infraestrutura controlada pela VoxCivis em território brasileiro. Não há transferência a terceiros nesses componentes.
